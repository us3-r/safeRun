...
fn main(){
    ...
    if config.dry_run {
        println!("\n\x1b[0;33;1m[ DRY RUN ]\x1b[0m");
        println!("Path: {},\n Pattern: {},\n Ignore: {},\n Fast: {},\n Show: {},\n Dry Run: {}\n", config.path, config.pattern, config.ignore, config.fast, config.show, config.dry_run);
    }
        ...
        if contains {
            ...
            if path.is_file() {
                ...
                if result.matches.iter().map(|m| m.len()).sum::<usize>() > 0{
                    ...
                    if result.high {
                        println!("\n\x1b[0;31;1m[ HIGH ] It seems like a highly privileged information has been left in the codebase\n\t Change the way you implemented such data !\x1b[0m");
                    }
                    if result.mid {
...
fn parse_args() -> Config {
    ...
    // If I remember correctly the long lines bellow are used to get the path of the patterns.txt and ignore.txt files from the exe path
    if &pattern == "None" {
